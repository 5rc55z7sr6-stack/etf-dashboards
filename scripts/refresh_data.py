"""
ETF Rotation Desk — Signal Refresh Script
Runs every 30 min via GitHub Actions.
Fetches OHLCV for all ratio tickers, computes SMA20/SMA50 signals,
writes data.json to repo root for instant page load.
"""
...
# ── Ratio definitions (mirrors ETF.html) ──────────────────────────────────────
CORE_R = [
...
SMA_S = 20   # short SMA
SMA_L = 50   # long  SMA
LOOKBACK = "1y"   # 1 year of daily data is plenty for SMA50

# ── Helpers ───────────────────────────────────────────────────────────────────
def classify(curr, s20, s50, inv=False):
    above20   = curr > s20
    s20_above_s50 = s20  > s50
    if above20 and s20_above_s50:
        sig = "strong-bull"
    elif above20:
        sig = "bull"
    elif (not above20) and (not s20_above_s50):
        sig = "strong-bear"
    else:
        sig = "bear"
    if inv:
        ...
        sig = flip[sig]
    bullish = sig in ("strong-bull", "bull")
    return {"sig": sig, "bullish": bullish}
...
print(f"Fetching {len(tickers)} tickers: {', '.join(sorted(tickers))}")
...
if isinstance(raw.columns, pd.MultiIndex):
    ...
    # Single ticker (shouldn't happen but handle it)
    t = tickers[0]
    s = raw["Close"].dropna()
    if len(s) >= SMA_L + 5:
        closes[t] = s

print(f"Got price data for {len(closes)}/{len(tickers)} tickers")
...
for r in ALL_R:
    ...
    if num_t not in closes or den_t not in closes:
        ...
        print(f"  SKIP {r['key']} — missing ticker data")
        continue
    ...
    if len(common_idx) < SMA_L + 5:
        results.append({**r, "error": True})
        print(f"  SKIP {r['key']} — not enough aligned rows ({len(common_idx)})")
        continue
    ...
    print(f"  OK   {r['key']:12s}  curr={curr:.4f}  s20={s20:.4f}  s50={s50:.4f}  → {cl['sig']}")
...
print(f"\n✅ data.json written — {ok_count}/{len(results)} ratios OK, ts={output['generated']}")
